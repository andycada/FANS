...
#MEJILLON (M. edulis) en areas (BBF, BBE, PP)---------------------#
  ...
  theme_set(theme_bw())
corM<- cor %>% filter(organism == "M. edulis" ) 


#
# f(x) = d / (1+exp(b(x - e)))
#
model1 <- drm(Conc ~ Dias, fct=lgaussian() , data = corM)
plot(model1)
summary(model1)
model2 <- drm(Conc ~ Dias, fct=multi2() , data = corM)
plot(model2)
AIC(model1,model2)
...
require(propagate)
corM[which.max(corM$Conc),]
#
# Laplace
#
nonlin <- function(t, a, b, c) { a * (exp(-(abs(t-c) / b))) }
nlsfit1 <- nls(Conc ~ nonlin(Dias, a, b, c), data=corM, start=list(a=55, b=2, c=75),algorithm = "port")
summary(nlsfit1)
nlsfit1 <- nls(Conc ~ nonlin(Dias, a, b, c), data=corM, start=list(a=3500, b=5, c=75),algorithm = "port")
pred <- predict(nlsfit1, newdata = data.frame(Dias = corM$Dias))
#pred$summary
df <- cbind(corM, pred)
