...
zooplankton <- zooplankton_example%>%
...
str(zoo_train)
str(zoo_test)# pueden tener distinto tamanio 
...
zoo_plot_data <- expand.grid(day = 1:365, 
                             ...
                             year_f = 1980)
view(zoo_plot_data)
str(zoo_plot_data) #de los datos pares (train) crea un subset para usar predict
...
zoo_modS_fit <- predict(zoo_comm_modS, 
                        zoo_plot_data, 
                        se.fit = TRUE, 
                        exclude = "s(taxon,year_f)") # ver cual es este temrino para mi
...
zoo_plot <- ggplot(zoo_plot_data) +  ## esto sale de usar predict ( subset para un solo year)
  ...
  geom_ribbon(aes(x=day,
                  ...
                  fill = model),
              alpha=0.2)+
  geom_point(data= zoo_train, aes(x = day, y = density_adj),size=0.06)+ # puntos de todos los datos usados para modelar
  geom_point(data= zoo_test, aes(x = day, y = density_adj),             # puntos de los datos q quedaron afuera para testear 
             size=0.06,col="grey")+
...
zoo_plot  # las lineas son hechas en base a predict ( a partir de un subset) para ambos modelos, los puntos son del set de datos usado para modelar y del otro tb 
