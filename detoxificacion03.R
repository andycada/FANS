...
AIC(model6GSA,model6GA,model6SA) # modelo S es el mejor 
...
## GI (seria mas un I porque no tiene global smoother )
...
## PROBANDO GI con global smoother **lo toma bien pero no grafica (PAPER)
...
# GI Plot prediccion (model4GIBBE)
#
pred <- tibble(Days=rep(seq(from=min(DETOX2BBE$Days), to=max(DETOX2BBE$Days)),2), organism =rep( unique(DETOX2BBE$organism),each=max(DETOX2BBE$Days)+1) )
...
# GI Plot prediccion (model4GIBBE2)
pred <- tibble(Days=rep(seq(from=min(DETOX2BBE$Days), to=max(DETOX2BBE$Days)),2), organism =rep( unique(DETOX2BBE$organism),each=max(DETOX2BBE$Days)+1) )
